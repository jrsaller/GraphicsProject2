...
void display(void)
{
	...
	for (size_t q = 0; q < circles.size();q++) {
		...
		if (circles[q].getY() - circles[q].getRadius() + circles[q].getDeltaY() < 0)
			//dy = -dy;
			circles[q].setDeltaY(-circles[q].getDeltaY());
		circles[q].setX(circles[q].getX() + circles[q].getDeltaX());
		circles[q].setY(circles[q].getY() + circles[q].getDeltaY());
		//x += dx;
		//y += dy;
...
void InitializeMyStuff()
{
	srand(time(0));
	for (int i = 0; i < 10;i++) {
		double radius = rand() % 30 + 8;
		double x = rand() % (int)(screen_x - radius);
		double y = rand() % (int)(screen_y - radius);
		double deltaX = rand() % 100 / 100.0;
		double deltaY = rand() % 100 / 100.0;
